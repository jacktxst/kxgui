...
// - fix: buffer overflow crashes
// - fix: multiple scroll bars appear at once even if only one of them is active
// - fix: elements will "wrap" even when it doesnt help.
// - add sideways scroll
// - add drag support
...
typedef struct 
{
   ...
   ivec2 _cursor; // position at which to try to draw the next component
   // local to parent container, not screen space

   f32 _tallest; // tracks the height of the tallest element in a line
   // so that the cursor is moved down by an appropriate
   // amount on the next new line.

   ivec2 _next_cursor;

   struct kxgui_rect _parent_window; // screen space
   // position and size of the currently active window / container

   struct kxgui_rect _clip; // screen space clip rect -
